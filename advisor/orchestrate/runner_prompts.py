...
_SCOPE_ANCHOR_BLOCK = (
    ...
    "**Drift is not a soft warning — it is mechanically discarded "
    "work.** Findings on a file outside your assigned batch are dropped "
    "by the verifier before the advisor ever reads them (the parser "
    "filters every finding against your batch). A drifted finding "
    "scores zero: you spent the turn, the codebase got nothing. There "
    "is no partial credit for a good bug found in the wrong file — flag "
    "it in one line and move back. And drift escalates: the first "
    "off-batch anchor gets you a REDIRECT; a **second drift on the same "
    "assignment gets you rotated out** (handoff brief, then "
    "``shutdown_request``) and a fresh runner takes over. Two strikes, "
    "not infinite reminders. Staying in-batch is the single cheapest "
    "thing you can do to be useful.\n\n"
...
def build_runner_pool_prompt(runner_id: int, config: TeamConfig) -> str:
    ...
    return (
        ...
        "Ping format:\n"
        f"    SendMessage(to='team-lead', message='CONTEXT_PRESSURE — "
        f"runner-{runner_id}: N fixes, M reads, recommend rotation')\n"
