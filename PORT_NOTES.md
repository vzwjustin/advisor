...
| `src/history.rs` | `history.py` | `HistoryEntry`, `load_recent[_findings]`, `file_repeat_scores` (exp decay) + `file_repeat_counts`, `summarize`, `format_history_block`, `append_entries`, `new_run_id`, `entry_now` | **Golden JSON** vs Python (scores/counts/summarize/format) + end-to-end `history`/`plan` parity |
...
| `advisor history-append` (write path) | ✓ | ✓ | **IDENTICAL** written `history.jsonl` bytes (default `json.dumps` separators) | `scripts/parity_check.sh` |
...
1. **`checkpoint.py` save** + wire `plan --checkpoint`/`--resume`.
2. The remaining `plan`/CLI surfaces (`--exclude`, `--output`, `--sarif` on
   ...
   install/doctor/web. not yet wired (documented gaps): `--since/--staged/--branch`
   (needs `git_scope.py`), `--estimate`/`--pricing` (needs full `cost.py`),
   `--checkpoint`/`--resume` (needs `checkpoint.py`), `--sarif`, `--exclude`,
   `--output`, and **Pretty (non-JSON) `plan` output
